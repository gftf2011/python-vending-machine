...
def test_should_raise_exception_if_quantity_is_negative():
    ...
    qty = -1
    with pytest.raises(
        InvalidProductsQtyException, match="quantity of products can not be negative"
    ):
        ProductEntity.create(
            "b9651752-6c44-4578-bdb6-883d703cbff5", "Hersheys", qty, "00", 0
        )
...
def test_should_raise_exception_if_unit_price_is_negative():
    ...
    unit_price = -1
    with pytest.raises(
        InvalidProductsPriceException, match="price of products can not be negative"
    ):
        ProductEntity.create(
            "b9651752-6c44-4578-bdb6-883d703cbff5", "Hersheys", 0, "00", unit_price
        )


def test_should_get_id():
    """Function to test if id property is called"""
    sut = ProductEntity.create(
