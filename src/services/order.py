...
class OrderService(IOrderService):
    ...
    def __find_product(self, product_id: str, products: list[ProductEntity]) -> Optional[ProductEntity]:
        for product in products:
            if product_id == product.get_id().get_value():
                return product
        return None
    ...
    async def create(self, input: CreateOrderInputDTO) -> CreateOrderOutputDTO:
        ...
        product_order: ProductEntity = ProductEntity.create(product_found.get_id().get_value(), product_found.get_name(), input.product_qty, product_found.get_code(), product_found.get_unit_price())
        order_item: OrderItemEntity = OrderItemEntity.create_new(UUIDValueObject.create_new().get_value(), product_order)
        order: OrderEntity = OrderEntity.create_new(UUIDValueObject.create_new().get_value(), input.machine_id, [order_item])

        await self.__order_repo.save(order)

        return CreateOrderOutputDTO(order.get_id().get_value())
